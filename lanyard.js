...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  let cardBody = null;
  let cardMesh = null;
  let cardSize = new THREE.Vector3(1.5, 2.2, 0.1);

  const segmentCount = 10; // Sedikit dikurangi biar lebih stabil
  const segmentLength = 0.22;
  const strapWidth = 0.25;

  const textureLoader = new THREE.TextureLoader();
  let strapTexture = null;
  try {
    strapTexture = await new Promise((resolve, reject) => {
      textureLoader.load('./3D-Lanyard/textures/strap-texture.png', (texture) => {
  ...
  const topAnchorPos = { x: 0, y: 2.8, z: 0 };
  ...
  for (let i = 0; i < segmentCount; i++) {
    ...
    // ✅ STABILITY: Damping Medium-High
    bodyDesc.setLinearDamping(1.5);
    bodyDesc.setAngularDamping(1.5);

    const body = world.createRigidBody(bodyDesc);
    
    const colliderDesc = RAPIER.ColliderDesc.ball(0.06);
    colliderDesc.setMass(0.5);
    colliderDesc.setCollisionGroups(0); // Disable self-collision
    
    world.createCollider(colliderDesc, body);
    ropeBodies.push(body);

    // ✅ FIX UTAMA: REVOLUTE JOINT (Engsel)
    // Cuma bisa ayun kiri-kanan (sumbu X), gak bisa muter gila-gilaan
    // Syntax: revolute(anchor1, anchor2, axis)
    const jointParams = RAPIER.JointData.revolute(
      { x: 0, y: i === 0 ? 0 : -segmentLength, z: 0 }, // Anchor di parent
      { x: 0, y: 0, z: 0 },                             // Anchor di child
      { x: 1, y: 0, z: 0 }                              // Axis rotasi X
    );
  ...
  function updateRibbon(curvePoints) {
    ...
    for (let i = 0; i <= ribbonSegments; i++) {
      const pointIndex = Math.min(i * (points.length - 1) / ribbonSegments, points.length - 1);
      const p = points[Math.floor(pointIndex)];
      
      let tangent;
      if (i === 0) {
        tangent = points.length > 1 ? new THREE.Vector3().subVectors(points[1], points[0]).normalize() : new THREE.Vector3(0, -1, 0);
      } else if (i === ribbonSegments) {
        tangent = new THREE.Vector3().subVectors(points[points.length - 1], points[points.length - 2]).normalize();
      } else {
        const prevIdx = Math.max(0, Math.floor(pointIndex) - 1);
        const nextIdx = Math.min(points.length - 1, Math.floor(pointIndex) + 1);
        tangent = new THREE.Vector3().subVectors(points[nextIdx], points[prevIdx]).normalize();
      }

      // ✅ FIX: Ribbon orientation stabil
  ...
  try {
    ...
    cardBodyDesc.setLinearDamping(2.0);
    cardBodyDesc.setAngularDamping(2.0); // Tinggi biar gak gila-gilaan

    cardBody = world.createRigidBody(cardBodyDesc);
    
    // ✅ FIX CARD GEPENG: Lock Y dan Z rotation
    // Biarin cuma muter X (maju mundur) biar selalu kelihatan full face
    cardBody.setEnabledRotations(true, false, false, true); 

    const cardColliderDesc = RAPIER.ColliderDesc.cuboid(cardSize.x / 2, cardSize.y / 2, cardSize.z / 2);
    cardColliderDesc.setMass(2.0);
    world.createCollider(cardColliderDesc, cardBody);

    scene.add(cardMesh);

    // Joint Card (Revolute juga biar konsisten)
    const cardJointParams = RAPIER.JointData.revolute(
      { x: 0, y: -segmentLength * 0.9, z: 0 },
      { x: 0, y: cardSize.y / 2 - 0.05, z: 0 },
      { x: 1, y: 0, z: 0 } // Axis X
    );
    ...
    const fallbackGeo = new THREE.BoxGeometry(cardSize.x, cardSize.y, cardSize.z);
    ...
    cardBodyDesc.setTranslation(0, cardY, 0);
    cardBodyDesc.setLinearDamping(2.0);
    cardBodyDesc.setAngularDamping(2.0);
  ...
  const onPointerDown = (e) => {
    ...
    if (cardMesh) {
      ...
      if (intersects.length > 0) {
        ...
        // Drag pakai Revolute juga biar stabil
        const jointParams = RAPIER.JointData.revolute(
          { x: 0, y: 0, z: 0 },
          { x: 0, y: 0, z: 0 },
          { x: 1, y: 0, z: 0 }
  ...
  const animate = () => {
    ...
    if (cardBody) {
      ...
      curvePoints.push(new THREE.Vector3(t.x, t.y + cardSize.y / 2 - 0.05, t.z));

      // ✅ FIX CARD GEPENG: Clamp rotation biar gak muter balik
      const rot = cardBody.rotation();
      
      // Paksa rotasi Y dan Z tetap 0 (biar card selalu ngadep kamera)
      // Cuma biarin rotasi X (maju mundur)
      _cardQuat.set(rot.x, 0, 0, 1); 
      
      // Normalize quaternion biar aman
      _cardQuat.normalize();
      
      cardMesh.quaternion.slerp(_cardQuat, 0.2);
